...
static inline tsk_id      tsk_create( unsigned prio, fun_id state, unsigned size ) { return (tsk_id) OSCall(os_tsk_create, prio, state, size); }
...
static inline void        tsk_start( tsk_id tsk ) { OSCall(os_tsk_start, tsk); }

// zatrzymanie aktualnego procesu (usuniêcie z listy zadañ)
              void     os_tsk_stop( void );
static inline void        tsk_stop( void ) { OSCall(os_tsk_stop); }

// zatrzymanie procesu 'tsk'
              void     os_tsk_kill( tsk_id tsk );
static inline void        tsk_kill( tsk_id tsk ) { OSCall(os_tsk_kill, tsk); }
...
static inline void        tsk_prio( unsigned prio ) { OSCall(os_tsk_prio, prio); }

// zawieszenie wykonywania aktualnego procesu do czasu 'time'
              unsigned os_tsk_sleepUntil( unsigned time );
static inline unsigned    tsk_sleepUntil( unsigned time ) { return OSCall(os_tsk_sleepUntil, time); }

// zawieszenie wykonywania aktualnego procesu na czas 'delay'
              unsigned os_tsk_sleepFor( unsigned delay );
static inline unsigned    tsk_sleepFor( unsigned delay ) { return OSCall(os_tsk_sleepFor, delay); }
...
static inline void        tsk_resume   ( tsk_id tsk, unsigned event ) { OSCall(os_tsk_resume, tsk, event); }
